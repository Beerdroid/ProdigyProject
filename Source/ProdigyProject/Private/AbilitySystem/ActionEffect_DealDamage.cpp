...
bool UActionEffect_DealDamage::Apply_Implementation(const FActionContext& Context) const
{
	...
	bool bOk = IActionAgentInterface::Execute_ModifyAttributeCurrentValue(
	...
	if (IsValid(Context.Instigator)) WorldContextObject = Context.Instigator;
	else if (IsValid(Context.TargetActor)) WorldContextObject = Context.TargetActor;

	UActionCueLibrary::PlayHitCue(
		WorldContextObject,
		Context.TargetActor,
		Context.Instigator,
		Damage,
		/*OptionalWorldLocation*/ FVector::ZeroVector
	);
